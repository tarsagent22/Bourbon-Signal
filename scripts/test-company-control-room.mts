...
assert.doesNotMatch(page, /State engine health|#state-engines|<h2>Operating health<\/h2>|#operations/);
assert.match(page, /<details id="paid-retention" className="cr-section cr-collapsible"/);
for (const label of ["Payment problems", "Members needing help", "Coverage requests", "Glasses to ship", "Retailer requests"]) assert.match(page, new RegExp(label));
assert.doesNotMatch(page, /ControlRoomActionCenter|Owner work queue|close the loop/i);
