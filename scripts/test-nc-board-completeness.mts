...
assert.equal((refreshWorkflow.match(/!inputs\.states \|\| contains\(inputs\.states, 'NC'\)/g) || []).length, 1, "scheduled/full refreshes must collect NC from the official board directory");
assert.match(refreshWorkflow, /Verify North Carolina scheduled continuity or isolate a safe partial fallback[\s\S]*if: \$\{\{ !inputs\.states \}\}[\s\S]*scheduled-state-verification\.mjs verify --state=NC -- npm run verify:nc -- --allow-safe-partial-fallback/, "scheduled refreshes must isolate failed NC verification against the safe retained partition");
assert.match(refreshWorkflow, /Verify North Carolina targeted recovery strictly[\s\S]*inputs\.states && contains\(inputs\.states, 'NC'\)[\s\S]*npm run verify:nc/, "targeted NC recovery must remain strict");
assert.match(refreshWorkflow, /Verify North Carolina scheduled official directory or isolate its partition[\s\S]*scheduled-state-verification\.mjs verify --state=NC -- npm run verify:nc-directory/, "scheduled directory verification must isolate only NC");
assert.match(refreshWorkflow, /Verify North Carolina targeted official directory strictly[\s\S]*inputs\.states && contains\(inputs\.states, 'NC'\)[\s\S]*run: npm run verify:nc-directory/, "targeted NC directory verification must remain strict");
