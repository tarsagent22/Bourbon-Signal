...
assert.equal((refreshWorkflow.match(/!inputs\.states \|\| contains\(inputs\.states, 'NC'\)/g) || []).length, 2, "scheduled/full refreshes must collect NC and verify the board directory");
assert.match(refreshWorkflow, /Verify North Carolina scheduled continuity or isolate a safe partial fallback[\s\S]*if: \$\{\{ !inputs\.states \}\}[\s\S]*npm run verify:nc -- --allow-safe-partial-fallback/, "scheduled refreshes must verify safe NC continuity");
