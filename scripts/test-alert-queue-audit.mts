...
const query = { query: async (text: string, params: unknown[], options?: unknown) => {
  ...
  return [{ delivered_rows: "9", unique_recipients: "4", onsite_count: "2", email_count: "5", sms_count: "2", repeated_identity_groups: "0", repeated_payload_groups: "0", repeated_underlying_bottle_groups: "3", latest_delivery_at: "2026-07-10T14:20:00.000Z", latest_repeated_underlying_bottle_at: "2026-07-10T14:10:00.000Z" }];
} } as never;
...
assert.equal(result.latestDeliveryAt, "2026-07-10T14:20:00.000Z");
assert.equal(result.latestRepeatedUnderlyingBottleAt, "2026-07-10T14:10:00.000Z");
...
assert.match(String(captured[0]), /max\(delivered_at\)/i);
assert.match(String(captured[0]), /latest_repeated_underlying_bottle_at/i);
assert.doesNotMatch(JSON.stringify(result), /user-|@|stableMatchKey|bottleName|locationName/i, "audit output must remain aggregate-only");
