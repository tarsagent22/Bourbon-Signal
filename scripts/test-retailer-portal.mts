...
assert.match(portal, /repository\.listStores\(userId, true\)/);
assert.match(portal, /const stores = allStores\.filter\(\(store\) => store\.status === "verified"\)/, "only verified locations may be offered for posting");
assert.match(portal, /Each new location requires independent verification/);
...
assert.match(portal, /allStores\.filter\(\(store\) => !store\.isPrimary\)/, "pending locations remain visible in profile without becoming publishable");
