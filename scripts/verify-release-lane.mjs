...
function sharedReleaseLaneDirectory() {
  return path.join(canonicalHermesHome(), 'kanban', 'boards', 'bourbon-signal-coverage', 'release-lane');
}
...
async function createCoveragePullRequest({ expectedMain, mainSha, pulls }) {
  ...
  if (pulls.length === 0) {
    ...
    if (process.argv.includes('--push-head')) {
      await assertAuthorityCapabilityAbsentFromGit(jobKey, {
        baseSha: expectedMain,
        headSha: expectedHead,
        headRef: head,
      ...
      commandOutput('git', ['push', '--set-upstream', 'origin', `${head}:${head}`]);
    }
