...
assert.match(nativeSignalFeed, /placeholder="Search bottle name"/, "bottle search must stay directly below the source toggle");
assert.match(nativeSignalFeed, /label="State"/, "state selection must stay directly on the feed");
assert.match(nativeSignalFeed, /filters\.state \? <OptionChooser compact label=/, "area selection must remain hidden until a state is selected");
