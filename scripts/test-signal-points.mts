...
test("schema, migration, encrypted backup, APIs, drawer, and owner queue are wired", () => {
  ...
  assert.match(ownerBoard, /spa-leaderboard/);
  assert.match(ownerBoard, /addEventListener\("toggle"/);
  assert.match(ownerBoard, /if \(active\) void load\(\)/);
  assert.doesNotMatch(ownerBoard, /All member balances|Search members|Includes members with a zero balance/);
