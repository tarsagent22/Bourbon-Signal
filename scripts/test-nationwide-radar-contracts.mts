...
test("referral, push, geography, and native UI source contracts are additive", () => {
  ...
  const mobileTypes = readFileSync(new URL("../apps/mobile/src/api/types.ts", import.meta.url), "utf8");
  const mobileClient = readFileSync(new URL("../apps/mobile/src/api/client.ts", import.meta.url), "utf8");
  const pushRegistration = readFileSync(new URL("../apps/mobile/src/push/push-registration.ts", import.meta.url), "utf8");
  ...
  assert.match(radar, /SafeAreaView/);
  assert.match(radar, /useSafeAreaInsets/);
  assert.match(radar, /paddingTop:\s*Math\.max\(insets\.top,/);
  assert.match(radar, /style=\{styles\.levelScroller\}/);
  ...
  assert.match(radar, /Adding a local filter replaces statewide monitoring\./);
  assert.match(radar, /monitoringScopesChanged/);
  assert.match(radar, /radarLocalityDisplayName/);
  ...
  assert.match(geography, /subtitle:\s*entry\.subtitle/);
  assert.match(geography, /name:\s*entry\.name/);
  assert.match(geography, /displayName:\s*entry\.level\s*===\s*"city"\s*\?\s*geographyDisplayName/);
  assert.match(geography, /store\.city/);
  assert.match(geography, /store\.address/);
  assert.match(mobileTypes, /displayName\?:\s*string/);
  assert.match(mobileTypes, /subtitle\?:\s*string/);
  assert.match(radar, /label:\s*row\.name/);
  assert.match(radar, /row\.displayName/);
