...
assert.match(worker, /source_access_denied/, 'the worker must distinguish the explicit OHLQ/LESC denial from generic browser readiness failures');
assert.match(worker, /cooldownActive:\s*true/, 'the worker must expose its persistent denial cooldown as an active blocked state');
