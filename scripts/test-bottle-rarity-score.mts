...
assert.match(bibleSource, /const approvedBottles: BibleBottleInput\[\] = includeApprovedCatalog[\s\S]*?\? await listApprovedBottles\(\)\.catch/, "normal Bible builds must retain durable approved additions behind an explicit inclusion guard");
assert.match(bibleSource, /mergeBottleCatalogSources\(\[\s*await readEngineBibleBottles\(\),\s*readInventoryBibleBottles\(\),\s*approvedBottles,\s*SEED_BOTTLES,\s*\]\)/, "engine alert data must remain lower authority than customer-facing inventory, approved additions, and curated rarity profiles");
