...
assert.ok(['useful', 'useful_retained_not_due'].includes(state.status), `Georgia status ${JSON.stringify(state.status)} is not release-useful.`);
assert.equal(state.stale, false, `Georgia retailer inventory must not publish as stale: ${state.staleReason || 'stale=true'}`);
...
assert.ok(georgiaAlerts.every((alert) => alert.eligibleForEmail === false && alert.eligibleForSms === false), 'Georgia activation baselines must remain on-site only; outbound alerts require a later source-backed change.');
