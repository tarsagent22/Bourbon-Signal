...
export function verifyMississippiResearchFoundation() {
  ...
  assert.equal(extensionEvidence.contractGitTag, 'evidence/ms-tupelo2go-contract-2026-07-30');
  assert.match(extensionEvidence.landedGitCommit, /^[a-f0-9]{40}$/u);
  const taggedContractCommit = execFileSync('git', ['rev-parse', `${extensionEvidence.contractGitTag}^{commit}`], { encoding: 'utf8' }).trim();
  assert.equal(taggedContractCommit, extensionEvidence.contractGitCommit);
  const committedContract = JSON.parse(execFileSync('git', ['show', `${taggedContractCommit}:engine/data/state-expansion-evidence/MS-tupelo2go-contract-2026-07-30.json`], { encoding: 'utf8' }));
  assert.deepEqual(committedContract, extensionContract);
  const contractHistoryCommittedAt = execFileSync('git', ['show', '-s', '--format=%cI', taggedContractCommit], { encoding: 'utf8' }).trim();
  assert.equal(Date.parse(extensionEvidence.contractGitCommittedAt), Date.parse(contractHistoryCommittedAt));
  const landedContract = JSON.parse(execFileSync('git', ['show', `${extensionEvidence.landedGitCommit}:engine/data/state-expansion-evidence/MS-tupelo2go-contract-2026-07-30.json`], { encoding: 'utf8' }));
  const landedEvidence = JSON.parse(execFileSync('git', ['show', `${extensionEvidence.landedGitCommit}:engine/data/state-expansion-evidence/MS-tupelo2go-2026-07-30.json`], { encoding: 'utf8' }));
  assert.deepEqual(landedContract, extensionContract);
  assert.equal(landedEvidence.contractGitCommit, taggedContractCommit);
  assert.equal(landedEvidence.contractGitCommittedAt, extensionEvidence.contractGitCommittedAt);
  assert.equal(landedEvidence.contractDigest, extensionEvidence.contractDigest);
  assert.deepEqual(landedEvidence.observed, extensionEvidence.observed);
  assert.deepEqual(landedEvidence.sourceCaptures, extensionEvidence.sourceCaptures);
  assert.deepEqual(landedEvidence.probeArtifacts, extensionEvidence.probeArtifacts);
  const landedCommittedAt = execFileSync('git', ['show', '-s', '--format=%cI', extensionEvidence.landedGitCommit], { encoding: 'utf8' }).trim();
  assert.equal(Date.parse(extensionEvidence.landedGitCommittedAt), Date.parse(landedCommittedAt));
  execFileSync('git', ['merge-base', '--is-ancestor', extensionEvidence.landedGitCommit, 'HEAD']);
  assert.ok(Date.parse(extensionEvidence.observed.startedAt) >= Date.parse(contractHistoryCommittedAt), 'Extension probe must start after the tagged acceptance contract was committed.');
  assert.ok(Date.parse(extensionEvidence.landedGitCommittedAt) >= Date.parse(extensionEvidence.observed.finishedAt), 'The immutable mainline evidence commit must postdate the completed extension probe.');
