...
const BOTTLECAPPS_PROBES = Object.freeze([
  ...
  {
    permitNumber: '051851',
    firstPartyDomains: ['madison-cellars.com'],
    platformIds: { siteId: '11754' },
    evidenceUrls: ['https://madison-cellars.com/'],
    probeStatus: 'blocked_by_source_policy',
  },
  {
    permitNumber: '044841',
...
export function buildMississippiSourceAtlas(universe) {
  ...
  return {
    ...
    researchMethod: {
      ...
      blockedSourceRule: 'Two exact GoToLiquorStore storefronts are manually readable but return 403 to the production HTTP client, so they remain blocked_by_source_policy rather than using an alternate-client or anti-bot bypass. Six BottleCapps first-party domains currently return DataDome 403; Oxford Wine & Spirits and Sipology fail DNS; two exact-permit Google Play discoveries are platform_probe_only/app_only_no_public_inventory. All twelve remain health-visible and nonalertable.',
