...
export const VIRGINIA_PRODUCTS = [
  ...
  { code: '021236', name: '1792 Small Batch Bourbon', limitedCaveat: false, verificationPriority: true },
  ...
  { code: '017913', name: 'Elijah Craig Toasted Barrel', limitedCaveat: false, verificationPriority: true },
  { code: '019880', name: "Michter's Us1 Small Batch Bourbon", limitedCaveat: false, verificationPriority: true },
  ...
  { code: '022092', name: 'Willett Pot Still Reserve Straight Bourbon', limitedCaveat: false, verificationPriority: true },
  { code: '016906', name: "Booker's Bourbon", limitedCaveat: true },
  { code: '016580', name: "Baker's Bourbon", limitedCaveat: false, verificationPriority: true },
  { code: '016577', name: "Baker's High Rye Bourbon", limitedCaveat: false, bootstrapPriority: true, targetStoreIds: ['40', '61', '82', '362'] },
  { code: '020384', name: 'Old Forester 1924 Craft Bourbon', limitedCaveat: true },
  { code: '020376', name: 'Old Forester 1920 Craft Bourbon', limitedCaveat: false, verificationPriority: true },
  { code: '020380', name: 'Old Forester Single Barrel Barrel Proof', limitedCaveat: true },
  { code: '016017', name: 'Heaven Hill Bottled In Bond Bourbon', limitedCaveat: false, verificationPriority: true },
  { code: '022288', name: 'Woodford Reserve Batch Proof Bourbon', limitedCaveat: true },
  { code: '022175', name: "Russell's Reserve 10 Year Bourbon", limitedCaveat: false, verificationPriority: true },
  { code: '022178', name: "Russell's Reserve Single Barrel Bourbon", limitedCaveat: false, verificationPriority: true },
  { code: '022179', name: 'Russells Reserve 13-year Bourbon', limitedCaveat: true },
  { code: '022199', name: 'Wild Turkey Rare Breed Bourbon', limitedCaveat: false, bootstrapPriority: true, targetStoreIds: ['40', '61', '82', '362'] },
  { code: '018434', name: 'Green River Full Proof Bourbon', limitedCaveat: false, bootstrapPriority: true, targetStoreIds: ['40', '61', '82', '362'] },
  { code: '018860', name: 'Larceny Barrel Proof', limitedCaveat: true },
  { code: '019239', name: 'Knob Creek 12 Year Bourbon', limitedCaveat: false, verificationPriority: true }
...
const VIRGINIA_PRODUCTS_PER_RUN = Math.max(15, Math.min(VIRGINIA_PRODUCTS.length, Number(process.env.BOURBON_SIGNAL_VA_PRODUCTS_PER_RUN || 15)));
const VIRGINIA_COLD_START_PRODUCTS_PER_RUN = Math.max(15, Math.min(VIRGINIA_PRODUCTS.length, Number(process.env.BOURBON_SIGNAL_VA_COLD_START_PRODUCTS_PER_RUN || 15)));
...
export function southCarolinaCityHiveBoundMerchantIds(products) {
  ...
  for (const product of Array.isArray(products) ? products : []) {
    for (const merchant of Array.isArray(product?.merchants) ? product.merchants : []) {
      for (const option of Array.isArray(merchant?.product_options) ? merchant.product_options : []) {
        ...
        const merchantId = typeof option.merchant_id === 'string' ? option.merchant_id.trim() : '';
        ...
        if (merchantId && productId && optionId) merchantIds.add(merchantId);
      }
    }
  }
  return [...merchantIds];
...
export function southCarolinaCityHiveMerchantEvidence(blobs, requestedMerchantId, limits = {}) {
  ...
  const maxNodes = safeLimits.maxNodes;
  const maxDepth = safeLimits.maxDepth;
  const nodeLimit = Number.isInteger(maxNodes) && maxNodes > 0 && maxNodes <= 50_000 ? maxNodes : 20_000;
  const depthLimit = Number.isInteger(maxDepth) && maxDepth >= 0 && maxDepth <= 20 ? maxDepth : 12;
  if (!Array.isArray(blobs) || typeof requestedMerchantId !== 'string' || !requestedMerchantId.trim()) {
    return { authoritative: false, merchants: [], options: [], truncated: false };
  }
  const requested = requestedMerchantId.trim();
  const normalizeAddress = (value) => String(value || '').toLowerCase().replace(/[^a-z0-9]+/gu, ' ').trim().replace(/\s+usa$/u, '');
  const merchants = [];
  const options = [];
  const seen = new WeakSet();
  const queue = [];
  let examinedEdges = 0;
  const enqueue = (value, depth, role, rootIndex, productContext = null) => {
    if (!value || typeof value !== 'object') return true;
    if (queue.length >= nodeLimit) return false;
    queue.push({ value, depth, role, rootIndex, productContext });
    return true;
  };
  const enqueueProperty = (object, key, depth, role, rootIndex, productContext = null) => {
    if (!Object.prototype.hasOwnProperty.call(object, key)) return true;
    examinedEdges += 1;
    return examinedEdges <= nodeLimit && enqueue(object[key], depth, role, rootIndex, productContext);
  };
  for (const key in blobs) {
    if (!Object.prototype.hasOwnProperty.call(blobs, key)) continue;
    examinedEdges += 1;
    if (examinedEdges > nodeLimit || !enqueue(blobs[key], 0, 'unknown', key)) return { authoritative: false, merchants: [], options: [], truncated: true };
  }
  for (let cursor = 0; cursor < queue.length; cursor += 1) {
    const entry = queue[cursor];
    if (!entry) continue;
    const { value, depth, role, rootIndex, productContext } = entry;
    if (!value || typeof value !== 'object' || seen.has(value)) continue;
    seen.add(value);
    if ((role === 'merchant' || role === 'merchant_configs') && !Array.isArray(value)
      && typeof value.id === 'string' && value.id === requested
      && value.address && typeof value.address === 'object' && !Array.isArray(value.address)) {
      const address = cityHiveAddressParts(value.address);
      if ((address.state || '').toUpperCase() === 'SC' || /,\s*SC\s+\d{5}/i.test(address.fullAddress || '')) {
        merchants.push({ value, rootIndex, address: normalizeAddress(address.fullAddress) });
      }
    }
    const parentProductId = productContext && typeof productContext === 'object' && !Array.isArray(productContext) ? productContext.id : null;
    const parentProductIdValid = parentProductId == null || parentProductId === ''
      || (typeof parentProductId === 'string' && parentProductId.trim() === value?.product_id);
    if (role === 'option' && parentProductIdValid && !Array.isArray(value)
      && typeof value.merchant_id === 'string' && value.merchant_id === requested
      && typeof value.product_id === 'string' && value.product_id.trim()
      && typeof value.option_id === 'string' && value.option_id.trim()
      && typeof value.full_address === 'string' && /,\s*SC\s+\d{5}/i.test(value.full_address)) {
      options.push({ value, rootIndex, address: normalizeAddress(value.full_address), product: productContext });
    }
    if (depth >= depthLimit) continue;
    if (!Array.isArray(value) && role === 'merchant') {
      if (!enqueueProperty(value, 'merchant', depth + 1, 'merchant', rootIndex)) return { authoritative: false, merchants: [], options: [], truncated: true };
      continue;
    }
    if (!Array.isArray(value) && role === 'product') {
      if (!enqueueProperty(value, 'product_options', depth + 1, 'product_options', rootIndex, value)
        || !enqueueProperty(value, 'merchants', depth + 1, 'merchants', rootIndex, value)) return { authoritative: false, merchants: [], options: [], truncated: true };
      continue;
    }
    if (!Array.isArray(value) && role === 'merchants') {
      if (!enqueueProperty(value, 'product_options', depth + 1, 'product_options', rootIndex, productContext)) return { authoritative: false, merchants: [], options: [], truncated: true };
      continue;
    }
    if (Array.isArray(value)) {
      for (const key in value) {
        ...
        examinedEdges += 1;
        if (examinedEdges > nodeLimit) return { authoritative: false, merchants: [], options: [], truncated: true };
        const childRole = role === 'product_options' ? 'option'
          : role === 'merchant_configs' ? 'merchant'
          : role === 'products' ? 'product'
          : role;
        const childProductContext = role === 'products' ? value[key] : productContext;
        if (!enqueue(value[key], depth + 1, childRole, rootIndex, childProductContext)) return { authoritative: false, merchants: [], options: [], truncated: true };
      }
      continue;
    }
    for (const key in value) {
      ...
      examinedEdges += 1;
      if (examinedEdges > nodeLimit) return { authoritative: false, merchants: [], options: [], truncated: true };
      const childRole = ['merchant_configs', 'merchantConfigs', 'merchants_config'].includes(key) ? 'merchant_configs'
        : key === 'products' ? 'products'
        : key === 'merchant' && role === 'merchant_configs' ? 'merchant'
        : 'unknown';
      const childProductContext = null;
      if (!enqueue(value[key], depth + 1, childRole, rootIndex, childProductContext)) return { authoritative: false, merchants: [], options: [], truncated: true };
    }
  }
  const matchedMerchants = merchants.filter((merchant) => options.some((option) => option.address === merchant.address)).map((merchant) => merchant.value);
  const matchedOptionRecords = options.filter((option) => merchants.some((merchant) => merchant.address === option.address));
  const matchedOptions = matchedOptionRecords.map((option) => option.value);
  return {
    authoritative: matchedMerchants.length > 0 && matchedOptions.length > 0,
    merchants: matchedMerchants,
    options: matchedOptions,
    optionRecords: matchedOptionRecords.map((record) => ({ option: record.value, product: record.product })),
    truncated: false,
  };
}

export function isAuthoritativeSouthCarolinaCityHiveMerchantPayload(input = {}) {
  ...
  if (typeof requestedMerchantId !== 'string' || !Array.isArray(configuredMerchantIds) || !Array.isArray(payloadMerchantIds)) return false;
  const requested = requestedMerchantId.trim();
  const strictIds = (values) => new Set(values
    .filter((value) => typeof value === 'string')
    .map((value) => value.trim())
    .filter(Boolean));
  return Boolean(requested)
    && hasProductPayload === true
    && strictIds(configuredMerchantIds).has(requested)
...
export function southCarolinaCityHiveProbePlan(sources = SC_CITYHIVE_SOURCES) {
  ...
  return probes.sort((left, right) => {
    const leftIsMyrtle = left.priority === 'myrtle';
    const rightIsMyrtle = right.priority === 'myrtle';
    if (leftIsMyrtle !== rightIsMyrtle) return leftIsMyrtle ? -1 : 1;
    if (!leftIsMyrtle) return 0;
    if (left.page !== right.page) return left.page - right.page;
    return SC_MYRTLE_CITYHIVE_MERCHANT_PRIORITY.get(left.merchantId) - SC_MYRTLE_CITYHIVE_MERCHANT_PRIORITY.get(right.merchantId);
  });
}
...
export async function readBoundedPrecisionResponse(response, {
  url = 'unknown',
  maxBytes = 2 * 1_024 * 1_024,
} = {}) {
  const boundedMaxBytes = Number(maxBytes);
  if (!Number.isFinite(boundedMaxBytes) || boundedMaxBytes <= 0) throw new Error('Precision response maxBytes must be a positive finite number.');
  const declaredHeader = response.headers?.get?.('content-length');
  const declaredBytes = declaredHeader == null ? null : Number(declaredHeader);
  if (Number.isFinite(declaredBytes) && declaredBytes > boundedMaxBytes) {
    await response.body?.cancel?.().catch(() => {});
    throw new Error(`Precision response from ${url} declared ${declaredBytes} bytes; maximum is ${boundedMaxBytes}`);
  }
  if (!response.body?.getReader) {
    const text = await response.text();
    if (Buffer.byteLength(text, 'utf8') > boundedMaxBytes) throw new Error(`Precision response from ${url} exceeded ${boundedMaxBytes} bytes`);
    return text;
  }
  ...
  try {
    while (true) {
      ...
      if (totalBytes > boundedMaxBytes) {
        ...
        throw new Error(`Precision response from ${url} exceeded ${boundedMaxBytes} bytes`);
      }
...
export function mergeSouthCarolinaCityHiveSignals(input = {}) {
  ...
  if (!Array.isArray(liveSignals) || !Array.isArray(completedMerchantIds)) return [];
  const completed = new Set(completedMerchantIds.filter((value) => typeof value === 'string'));
  const cityHiveIdentity = (signal) => {
    if (!signal || typeof signal !== 'object' || Array.isArray(signal)) return '';
    const merchantId = southCarolinaCityHiveSignalMerchantId(signal);
    const expectedChain = SC_CITYHIVE_MERCHANT_CHAIN.get(merchantId);
    if (!SC_CITYHIVE_INVENTORY_MERCHANT_IDS.has(merchantId)
      || typeof expectedChain !== 'string'
      || signal.sourceChain !== expectedChain
      || typeof signal.storeId !== 'string' || signal.storeId !== `${expectedChain}:${merchantId}`) return '';
    if (signal.eventType === 'cityhive_store_location') return `${signal.eventType}|${signal.storeId}`;
    if (signal.eventType !== 'cityhive_store_inventory_result'
      || typeof signal.productId !== 'string' || !signal.productId.trim()
      || typeof signal.optionId !== 'string' || !signal.optionId.trim()) return '';
    return `${signal.eventType}|${signal.storeId}|${signal.productId}|${signal.optionId}`;
  };
  const merged = new Map();
  for (const signal of liveSignals) {
    const key = cityHiveIdentity(signal);
    if (key) merged.set(key, signal);
  }
  for (const signal of cachedSouthCarolinaCityHiveSignals(cache, observedAt, { sourceStale: true })) {
    if (completed.has(southCarolinaCityHiveSignalMerchantId(signal))) continue;
    const key = cityHiveIdentity(signal);
    if (!key) continue;
...
async function collectSouthCarolinaCityHive(config, bible, observedAt) {
    ...
    evidence ||= southCarolinaCityHiveMerchantEvidence(blobs, merchantId);
          ...
          if (!completedMerchantIds.has(merchantId)) {
            ...
            roadblocks.push({
              ...
              source: source.sourceLabel,
              url,
              status: 'merchant_premise_mismatch',
              error: `CityHive response did not bind requested merchant ${merchantId} to a South Carolina premise.`,
              nextRoute: 'Review the merchant configuration and exact store address before retrying or changing the registry.'
            });
            continue;
          }
          ...
          for (const optionRecord of evidence.optionRecords) {
                ...
                if (parentProductId && parentProductId !== productId) continue;
                if (optionMerchantId !== merchantId || !sourceMerchantIds.has(optionMerchantId) || !productId || !optionId) continue;
                const fullAddress = typeof option.full_address === 'string' ? option.full_address : '';
  ...
  const liveInventoryProduced = liveSignals.some((signal) => signal.eventType === 'cityhive_store_inventory_result');
  const liveRoadblocks = [...roadblocks];
  signals.splice(0, signals.length, ...mergeSouthCarolinaCityHiveSignals({
    liveSignals,
    cache,
  ...
  if (liveInventoryProduced && completedMerchantIds.size === configuredProbeCount) {
    await writeSouthCarolinaCityHiveCache(liveSignals, liveRoadblocks);
  }
  const reconciled = reconcileCityHiveRateLimitsWithCache({
    roadblocks,
    sources: SC_CITYHIVE_SOURCES,
...
async function southCarolinaLiquorLibraryJsonFetch(url) {
  const res = await textFetch(url, { headers: { accept: 'application/json' }, timeoutMs: 24_000, maxBytes: SC_LIQUOR_LIBRARY_JSON_MAX_BYTES });
  const body = String(res.text || '');
  if (!res.ok) return { ok: false, status: res.status || 0, error: res.error || `HTTP ${res.status}`, payload: null };
  if (Buffer.byteLength(body, 'utf8') > SC_LIQUOR_LIBRARY_JSON_MAX_BYTES) {
    return { ok: false, status: res.status || 0, error: `Square JSON response exceeded ${SC_LIQUOR_LIBRARY_JSON_MAX_BYTES} bytes.`, payload: null };
  }
  try {
...
function isReusableSouthCarolinaLiquorLibraryCache(payload, nowMs = Date.now()) {
  const generatedMs = Date.parse(String(payload?.generatedAt || ''));
  const ageMs = nowMs - generatedMs;
  if (!Array.isArray(payload?.signals)
    || !Number.isFinite(generatedMs)
    || ageMs < -5 * 60_000
    || ageMs > SC_LIQUOR_LIBRARY_CACHE_MAX_AGE_MS) return false;
  const inventory = payload.signals.filter((signal) => signal?.eventType === 'retailer_store_inventory_result');
  const locations = payload.signals.filter((signal) => signal?.eventType === 'retailer_store_location');
  return inventory.length > 0
    && locations.length === 1
    && locations[0]?.storeId === `liquor-library:${LIQUOR_LIBRARY_SOURCE.locationId}`
    && locations[0]?.storeAddress === LIQUOR_LIBRARY_SOURCE.store.address
    && inventory.every((signal) => isSouthCarolinaLiquorLibraryInventory(signal, nowMs));
}

async function collectSouthCarolinaLiquorLibrary(config, bible, observedAt) {
  if (process.env.BOURBON_SIGNAL_SC_FORCE_LIQUOR_LIBRARY_LIVE !== '1') {
    try {
      const cached = JSON.parse(await readFile(SC_LIQUOR_LIBRARY_ARTIFACT_PATH, 'utf8'));
      if (isReusableSouthCarolinaLiquorLibraryCache(cached)) {
        return {
  ...
  const result = await collectLiquorLibraryInventory(config, bible, observedAt, {
    fetchJson: southCarolinaLiquorLibraryJsonFetch,
    sleepFn: sleep,
    matchBottle: (rawName) => cityHiveSafeBottleMatch(rawName, bible),
  });
  ...
  if (inventory.length > 0 && result.roadblocks.length === 0) {
    try {
      await mkdir(path.dirname(SC_LIQUOR_LIBRARY_ARTIFACT_PATH), { recursive: true });
      await writeFile(SC_LIQUOR_LIBRARY_ARTIFACT_PATH, JSON.stringify({ generatedAt: observedAt, signals: result.signals, roadblocks: [] }, null, 2));
    } catch (error) {
      result.roadblocks.push({ state: config.id, source: LIQUOR_LIBRARY_SOURCE.sourceLabel, url: LIQUOR_LIBRARY_SOURCE.categoryUrl, status: 'cache_write_failed', error: error instanceof Error ? error.message : String(error), nextRoute: 'Serve this verified live result and retry cache persistence on the next run.' });
    }
  }
  return result;
}
...
export function virginiaStoreSignals(product, json, config, bible, url, supportedStoreIds = null, origin = null) {
  const signals = [];
  const originStoreId = String(origin?.storeNumber || '').trim();
  const targetStoreIds = new Set((product.targetStoreIds || []).map(String));
  const requiresPremisesProof = targetStoreIds.size > 0;
  ...
  for (const store of rows) {
    ...
    if (!storeId || (supportedStoreIds && !supportedStoreIds.has(String(storeId)))
      || (requiresPremisesProof && !targetStoreIds.has(String(storeId)))) continue;
    const quantity = Number(store.quantity);
    if (!Number.isSafeInteger(quantity) || quantity < 0
      || (requiresPremisesProof && (!origin || !virginiaInventoryPremisesMatch(store, origin)))) continue;
    ...
    const verifiedAddress = requiresPremisesProof
      ? String(origin.address || '').trim()
      : store.address || [store.address1, store.address2, store.city, store.state, store.zip].filter(Boolean).join(', ');
    const verifiedCity = requiresPremisesProof ? String(origin.city || '').trim() : store.city || origin?.city || null;
    const verifiedZip = requiresPremisesProof
      ? String(origin.zip || '').match(/^\d{5}/)?.[0] || null
      : store.zip || origin?.zip || null;
    const verifiedLat = Number(requiresPremisesProof ? origin.lat ?? origin.latitude : store.latitude ?? origin?.lat);
    const verifiedLng = Number(requiresPremisesProof ? origin.lng ?? origin.longitude : store.longitude ?? origin?.lng);
    ...
    signals.push({
      ...
      lat: Number.isFinite(verifiedLat) ? verifiedLat : null,
      lng: Number.isFinite(verifiedLng) ? verifiedLng : null,
      latitude: Number.isFinite(verifiedLat) ? verifiedLat : null,
      longitude: Number.isFinite(verifiedLng) ? verifiedLng : null,
      ...
      sourceAvailabilityVerified: true,
      premisesVerified: true,
      sourcePremisesProofVersion: requiresPremisesProof ? 1 : null,
      virginiaCacheSchemaVersion: requiresPremisesProof ? 3 : 2,
      targetStoreIds: requiresPremisesProof ? [...targetStoreIds] : null,
      observedAt: base.fetchedAt,
      canAlertAsInventory: quantity > 0,
      canAlertAsWatch: true,
      ...
      evidence: `Virginia ABC reports ${quantity} bottle(s) of ${product.name} at Store ${storeId}${verifiedCity ? ` in ${verifiedCity}` : ''}. ${product.limitedCaveat ? 'Limited-availability products may be intentionally hidden or randomized outside authorized release windows.' : 'This is a normal-product inventory reading.'} Verify before driving.`,
      raw: {
        ...
        premisesVerified: true,
        sourcePremisesProofVersion: requiresPremisesProof ? 1 : null,
        premisesAuthority: requiresPremisesProof ? {
          source: 'virginia_arcgis',
          storeId: String(storeId),
          directoryAddress: String(origin.address || '').trim(),
          directoryCity: String(origin.city || '').trim(),
          directoryZip: String(origin.zip || '').match(/^\d{5}/)?.[0] || null,
          directoryLat: Number.isFinite(verifiedLat) ? verifiedLat : null,
          directoryLng: Number.isFinite(verifiedLng) ? verifiedLng : null,
          matchMethod: 'selected_official_directory_origin'
        } : null,
        sourcePremises: requiresPremisesProof ? {
          address: String(store.address || '').trim(),
          city: String(store.city || '').trim(),
          zip: String(store.zip || '').match(/^\d{5}/)?.[0] || null
        } : null,
        virginiaCacheSchemaVersion: requiresPremisesProof ? 3 : 2
      }
    });
...
function enrichVirginiaCachedSignal(signal, cacheGeneratedAt) {
  const parsedQuantity = Number(signal.quantity);
  const quantityValid = Number.isSafeInteger(parsedQuantity) && parsedQuantity >= 0;
  const quantity = quantityValid ? parsedQuantity : 0;
  const eventType = quantityValid
    ? signal.eventType || (quantity > 0 ? 'store_inventory_result' : 'store_inventory_out_of_stock')
    : 'store_inventory_out_of_stock';
  return {
    ...signal,
    quantity,
    reportedQuantity: quantity,
    eventType,
    availabilityStatus: quantityValid ? signal.availabilityStatus || (quantity > 0 ? 'in_stock' : 'out_of_stock') : 'out_of_stock',
    availabilityLabel: quantityValid ? signal.availabilityLabel || (quantity > 0 ? `${quantity} reported available` : 'Out of stock') : 'Out of stock',
    canAlertAsInventory: quantityValid ? signal.canAlertAsInventory ?? quantity > 0 : false,
    canAlertAsWatch: quantityValid ? signal.canAlertAsWatch ?? true : false,
...
async function collectVirginia(config, bible, options = {}) {
  ...
  try {
    ...
    const requestedPriorityStoreIds = [...new Set([
      '49',
      ...String(process.env.BOURBON_SIGNAL_VA_PRIORITY_STORE_IDS
  ...
  sanitizedCachedSignals = sanitizeVirginiaInventoryCacheSignals(rawCachedSignals, storeUniverseVerified ? stores : []);
  ...
  const requiredTargetStoreIds = String(process.env.BOURBON_SIGNAL_VA_REQUIRED_STORE_ID || '')
    .split(',').map((storeId) => storeId.trim()).filter(Boolean);
  const refreshPlan = virginiaRefreshPlan({
    cachedSignalCount: cachedSignals.length,
    recoveryBacklogCount: recoveryBacklogProductCodes.size,
    normalLimit: VIRGINIA_PRODUCTS_PER_RUN,
    coldLimit: VIRGINIA_COLD_START_PRODUCTS_PER_RUN,
    requiredStoreIds: requiredTargetStoreIds
  });
  const selectedProducts = selectVirginiaProductsForRefresh(storeUniverseVerified ? VIRGINIA_PRODUCTS : [], cachedSignals, nowMs, {
    maxProducts: refreshPlan.maxProducts,
    regularIntervalMs: VIRGINIA_REGULAR_REFRESH_MS,
    limitedIntervalMs: VIRGINIA_LIMITED_REFRESH_MS,
    force: process.env.BOURBON_SIGNAL_VA_FORCE_LIVE === '1' || refreshPlan.force
  });
  ...
  for (const product of selectedProducts) {
    const productTargetStoreIds = new Set((product.targetStoreIds || []).map(String));
    const productStores = productTargetStoreIds.size
      ? stores.filter((origin) => productTargetStoreIds.has(String(origin.storeNumber)))
      : stores;
    const productExpectedStoreIds = productTargetStoreIds.size ? productTargetStoreIds : expectedStoreIds;
    const productSignals = [];
    const seenSignalIds = new Set();
    const productErrors = [];
    for (let i = 0; i < productStores.length; i += batchSize) {
      throwIfVirginiaAborted(options.signal);
      if (sharedRateLimitState.tripped) {
        stopForRateLimit = true;
        break;
      }
      const batch = productStores
        .slice(i, i + batchSize)
      ...
      for (const result of results) {
        ...
        const originSignals = virginiaStoreSignals(product, result.value.json, config, bible, result.value.url, productExpectedStoreIds, result.value.origin);
      ...
      if (i + batchSize < productStores.length) await virginiaAbortableDelay(batchDelayMs, options.signal);
    }

    throwIfVirginiaAborted(options.signal);
    const coverage = evaluateVirginiaProductCoverage(productSignals, productExpectedStoreIds, {
      minimumExpectedStoreCount: productTargetStoreIds.size || 390
    });
