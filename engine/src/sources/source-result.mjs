...
export function createSourceSuccessResult({ adapter, value, startedAt, finishedAt, attemptCount, attempts = [], quarantined = false, schedule = null }) {
  ...
  return {
    ...
    recordsInspected: value?.recordsInspected ?? value?.metadata?.recordsInspected ?? null,
    usefulChanges: Number.isFinite(value?.usefulChanges) ? Math.max(0, value.usefulChanges) : 0,
    consecutiveUnchanged: Number.isFinite(value?.consecutiveUnchanged) ? Math.max(0, value.consecutiveUnchanged) : 0,
    error: null,
...
export function createSourceFailureResult({ adapter, error, previous = null, startedAt, finishedAt, attemptCount, attempts = [], schedule = null, quarantined = false }) {
  ...
  return {
    ...
    recordsInspected: previous?.recordsInspected ?? previous?.value?.recordsInspected ?? previous?.metadata?.recordsInspected ?? null,
