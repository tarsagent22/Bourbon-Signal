...
async function main() {
  ...
  assert((nc.roadblockCount || 0) <= 5, `NC roadblocks exceed threshold: ${nc.roadblockCount}`);
  ...
  console.log(`NC verified: ${nc.coverage.boardCount} boards, ${nc.coverage.withTrackedShipments} boards with tracked shipments, ${nc.coverage.withInventoryPages} inventory/release-capable boards, ${nc.signalCounts.nc_board_shipment_snapshot} shipment signals, ${nc.signalCounts.nc_statewide_warehouse_stock} warehouse stock signals, ${ncDrops.length} site drops, ${ncLocations.length} locations, ${nc.roadblockCount || 0} roadblocks.`);
}
