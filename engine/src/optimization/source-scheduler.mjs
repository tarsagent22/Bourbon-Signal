import { constrainFreshnessDeadline } from './freshness-deadline.mjs';
...
export function decideSourceSchedule(metrics = {}, options = {}) {
  ...
  const adaptiveCadenceMs = Math.round(clamp(base * valueMultiplier * failureMultiplier, min, max));
  const lastProbe = Date.parse(metrics.lastProbeAt || '');
  const now = Date.parse(options.now || new Date().toISOString());
  const anchor = Number.isFinite(lastProbe) ? lastProbe : now;
  const constraint = constrainFreshnessDeadline(Number.isFinite(lastProbe) ? lastProbe + adaptiveCadenceMs : now, metrics, options, now);
  const cadenceMs = Math.max(0, constraint.nextProbeMs - anchor);
  const nextProbeAt = new Date(constraint.nextProbeMs).toISOString();
  const due = now >= constraint.nextProbeMs;
  return {
    sourceId: metrics.sourceId || null,
    decision: metrics.disabled ? 'disabled' : due ? 'probe_now' : 'wait',
    cadenceMs,
    nextProbeAt,
    freshnessDeadlineAt: constraint.freshnessDeadlineAt,
    roiScore: Math.round(roiScore * 1000) / 1000,
    reasons: metrics.disabled ? ['disabled'] : [roiScore >= 0.6 ? 'high_roi' : roiScore < 0.2 ? 'low_roi' : 'normal_roi', ...(failureRate >= 0.5 ? ['failure_backoff'] : []), ...(Number(metrics.consecutiveUnchanged || 0) >= 4 ? ['unchanged_backoff'] : []), ...(constraint.freshnessLimited ? ['freshness_deadline'] : []), ...(constraint.freshnessDeferred ? ['freshness_deferred_by_source_policy'] : [])]
  };
}
