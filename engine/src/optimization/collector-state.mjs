...
export function decideCollectorProbe(metadata = {}, options = {}) {
  ...
  const storedNextProbe = asTime(metadata.nextProbeAt);
  const nextProbe = constrainFreshnessDeadline(storedNextProbe ?? now, metadata, options, now).nextProbeMs;
  ...
  if (!options.force && now != null && nextProbe != null && now < nextProbe) {
    return { decision: 'skip_not_due', headers: {}, nextProbeAt: new Date(nextProbe).toISOString() };
  }
...
export function updateCollectorMetadata(previous = {}, response = {}, options = {}) {
  ...
  const evidence = {
    ...
    ...(!failed ? Object.fromEntries(['lastConfirmedAt', 'freshnessDeadlineAt', 'freshnessMaxAgeMs']
      .filter((key) => response[key] != null).map((key) => [key, response[key]])) : {}),
    lastStatus: response.status ?? null,
    ...(response.retryAfterAt ? { retryAfterAt: response.retryAfterAt } : {}),
  };
  const nextProbe = constrainFreshnessDeadline(Date.parse(checkedAt) + cadenceMs, evidence, options, Date.parse(checkedAt));
  return {
    ...evidence,
    ...
    nextProbeAt: new Date(nextProbe.nextProbeMs).toISOString(),
