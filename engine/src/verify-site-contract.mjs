...
const required = ['manifest.json', 'stats.json', 'bottles.json', 'stores.json', 'locations.json', 'drops.json', 'events.json', 'alerts.json', 'state-quality.json', 'states/index.json'];
...
const partitionIndex = readJson('states/index.json');
const partitionDrops = [];
for (const expected of activeStates) {
  const entry = (partitionIndex.states || []).find((item) => item.state === expected);
  if (!entry) {
    fail(`states/index.json missing active state ${expected}.`);
    continue;
  }
