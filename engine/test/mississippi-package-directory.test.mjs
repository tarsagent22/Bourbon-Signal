...
test('region matching is exact and source atlas closes every research disposition', async () => {
  ...
  assert.equal(summary.inventoryCapable, 7);
  assert.equal(summary.blockedOrOfflineOrProbeOnly, 10);
  assert.equal(atlas.stores.filter((store) => store.platform === 'bottlecapps').length, 8);
