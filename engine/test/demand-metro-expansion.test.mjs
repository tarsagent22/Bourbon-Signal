...
test('lifecycle, collectors, verifiers, and CI expose all three demand-selected metros', () => {
  ...
  assert.match(workflow, /verify:demand-metros -- --structural-only/);
  ...
  assert.match(workflow, /Verify demand metro structure independently of state freshness[\s\S]{0,240}if:\s*\$\{\{ !inputs\.states \}\}[\s\S]{0,240}--structural-only/);
  ...
  assert.doesNotMatch(workflow, /verify:demand-metros[^\n]*--allow-fresh-retained-evidence/, 'scheduled demand-metro freshness must be enforced only by state-specific verifier wrappers');
