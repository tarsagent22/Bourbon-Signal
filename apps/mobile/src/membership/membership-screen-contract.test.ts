...
test("membership overview compares every tier with accurate pricing and native detail routes", () => {
  ...
  assert.match(screen, /<MembershipComparison/);
  const comparison = read("src/membership/MembershipComparison.tsx");
  assert.match(comparison, /MEMBERSHIP_COMPARISON_ROWS\.map/);
  assert.match(comparison, /Compare features/);
  assert.doesNotMatch(comparison, /ScrollView|numberOfLines|adjustsFontSizeToFit/);
  assert.doesNotMatch(screen, /ScrollView horizontal|Free is shown as a baseline/);
  assert.match(comparison, /useState\(false\)/);
  assert.match(comparison, /accessibilityState=\{\{ expanded: freeExpanded \}\}/);
  assert.match(comparison, /Both plans include/);
  assert.match(comparison, /Founder/);
  assert.match(comparison, /No alerts/);
