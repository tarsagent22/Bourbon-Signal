...
| Location — Precise Location | No collection by the current native contract | — | — | Optional foreground location may be used on-device for nearby-retailer suggestions after an explicit member action. It is not sent to the backend, background location is disabled, and manual retailer entry remains available. |
