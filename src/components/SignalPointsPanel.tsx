...
type EarningRow = { action: string; detail: string; points: string };
...
const EARNING_ROWS: EarningRow[] = [
  { action: "Post an unclassified sighting", detail: "A valid member sighting that remains active.", points: `+${SIGHTING_POINTS_BY_RARITY.unclassified}` },
  { action: "Post a limited sighting", detail: "A valid limited-bottle sighting that remains active.", points: `+${SIGHTING_POINTS_BY_RARITY.limited}` },
  { action: "Post an allocated sighting", detail: "A valid allocated-bottle sighting that remains active.", points: `+${SIGHTING_POINTS_BY_RARITY.allocated}` },
  { action: "Post a unicorn sighting", detail: "A valid unicorn-tier sighting that remains active.", points: `+${SIGHTING_POINTS_BY_RARITY.unicorn}` },
  { action: "Earn a new badge", detail: "Each First Sighting, Helpful Neighbor, Photo Finish, Spotter, Unicorn Hunter, Sharp Eye, Local Scout, Weekend Warrior, Clean Signal, or Streak badge tier.", points: `+${BADGE_POINTS_AWARD}` },
  { action: "Maintain a weekly sighting streak", detail: "Each consecutive qualifying week after the first.", points: `+${WEEKLY_STREAK_POINTS_AWARD}` },
  { action: "Refer a Free member", detail: "The first five Free referral awards per referrer.", points: `${REFERRAL_POINTS_BY_TIER.free} total` },
  { action: "Referral upgrades to Standard", detail: `The referral reaches ${REFERRAL_POINTS_BY_TIER.standard} total points; only the difference is added.`, points: `${REFERRAL_POINTS_BY_TIER.standard} total` },
  { action: "Referral upgrades to Barrel", detail: `The referral reaches ${REFERRAL_POINTS_BY_TIER.barrel} total points; only the difference is added.`, points: `${REFERRAL_POINTS_BY_TIER.barrel} total` },
  { action: "Referral upgrades to Bottled-in-Bond", detail: `The referral reaches ${REFERRAL_POINTS_BY_TIER["bottled-in-bond"]} total points; only the difference is added.`, points: `${REFERRAL_POINTS_BY_TIER["bottled-in-bond"]} total` },
];
...
export default function SignalPointsPanel(props: SignalPointsPanelProps) {
  ...
  const earningGuide = showEarningGuide ? <div className="points-guide-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeEarningGuide(); }}><section className="points-guide-sheet" role="dialog" aria-modal="true" aria-labelledby="points-guide-title">
    ...
    <div className="points-guide-table" role="table" aria-label="Every way to earn Signal Points">
      <div className="points-guide-table-head" role="row"><span role="columnheader">Action</span><span role="columnheader">Points</span></div>
      {EARNING_ROWS.map((row) => <div className="points-guide-row" role="row" key={row.action}><span role="cell"><strong>{row.action}</strong><small>{row.detail}</small></span><b role="cell">{row.points}</b></div>)}
    </div>
    <p>Removed or rejected sightings are reconciled. Free members can earn points; a paid membership is required only to redeem.</p>
  </section></div> : null;
