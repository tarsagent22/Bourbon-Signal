    ...
    <section className="points-preview" aria-label="Signal Points preview experience">
      ...
      {tab === "overview" ? <div className="points-tab-panel">
        <section className="points-focus-grid">
          <button className="points-feature-card" type="button" onClick={() => setTab("rewards")}>
            <span className="points-kicker">Within reach</span>
            <div className="reward-mark">{REWARD_MARKS[(availableReward || nextReward)?.key || ""] || "◈"}</div>
            <div><strong>{availableReward?.name || nextReward?.name || "Reward catalog"}</strong><small>{availableReward ? `${availableReward.points} points · ready to redeem` : nextReward ? `${nextReward.points} points` : "Explore everything available"}</small></div>
            <b>{availableReward ? "Ready" : `${Math.max(0, (nextReward?.points || data.balance) - data.balance)} left`} →</b>
          </button>
          <button className="points-feature-card badge-focus" type="button" onClick={() => setTab("badges")}>
            ...
            <div><strong>{nextBadge ? `${badgeLabel(nextBadge.id, nextBadge.label)}${nextBadge.tier ? ` · ${nextBadge.tier}` : ""}` : "Badge collection"}</strong><small>{nextBadge ? `${nextBadge.current}/${nextBadge.target} complete` : "Your visible achievements are earned"}</small></div>
            <b>View progress →</b>
          </button>
        </section>
