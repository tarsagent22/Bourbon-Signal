...
const REWARD_MARKS: Record<string, string> = {
  ...
  glencairn_pair: "G²",
  hoodie: "H",
  standard_membership_credit_month: "$3",
  barrel_membership_credit_month: "$6",
...
export default function SignalPointsPanel(props: SignalPointsPanelProps) {
  ...
  const membershipCredit = selected?.options.membershipCredit === true;
  ...
  const previewTabs: PreviewTab[] = ["overview", "rewards", "badges", "history"];
  function handleTabKeyDown(event: ReactKeyboardEvent<HTMLButtonElement>, index: number) {
    ...
    const nextIndex = event.key === "Home"
      ...
      : event.key === "End"
        ...
        : (index + (event.key === "ArrowRight" ? 1 : -1) + previewTabs.length) % previewTabs.length;
  ...
  const compactControl = props.compact ? (
    <button
    ...
    >
      <span className="section-copy">
        ...
        <span className="section-summary">{compactRewardStatus}</span>
      </span>
      <span className="section-arrow" aria-hidden="true"><span className="section-chevron" /></span>
    </button>
  ) : null;

  if (props.compact && !data) return (
    <section id="signal-points" className="signal-points-accordion" aria-label="Signal Points">
      ...
      <div id="signal-points-accordion-panel" className="signal-points-accordion-panel" role="region" aria-labelledby="dashboard-section-memberPoints" hidden={!props.expanded}>
        {error ? (
          <div className="signal-points-accordion-retry"><p>{error}</p><button type="button" disabled={loading} onClick={() => void load()}>{loading ? "Trying again…" : "Try again"}</button></div>
        ) : <DefaultLoading />}
      </div>
      <AccordionStyles />
    </section>
  ...
  if (!props.preview) return (
    <section className="signal-points-panel" aria-label="Signal Points rewards">
      ...
      <div className="signal-catalog"><h4>Reward catalog</h4><div>{data.catalog.map((item) => {
        ...
        return <article key={item.key}><strong>{item.name}</strong><span>{item.points} pts{item.fulfillmentType === "physical" ? " · U.S. shipping included" : item.options.membershipCredit === true ? " · automatically applied to your next bill" : " · digital, owner fulfilled"}</span>{item.key === "bourbon_shipping_gift_card_100" ? <button ref={caskersRestrictionsButton} className="caskers-restrictions-link" type="button" onClick={() => setShowCaskersRestrictions(true)}>Shipping restrictions</button> : null}<button type="button" disabled={disabled} onClick={() => selectReward(item)}>{item.inventoryRemaining === 0 ? "Out of stock" : !data.redemptionEligible ? "Paid membership required" : data.balance < item.points ? `${item.points - data.balance} pts to go` : "Redeem"}</button></article>;
      })}</div></div>
  ...
  const badgeCards = BADGES.map((badge) => {
    ...
    const earnedProgress = [...progress].reverse().find((item) => item.earned);
    const earned = Boolean(props.rewards?.badges.some((item) => badgeKey(item.id) === badge.id) || earnedProgress);
    const next = progress.find((item) => !item.earned) || earnedProgress || progress[0];
    const iconId = earnedProgress?.id || next?.id || badge.id;
    return { ...badge, earned, next, iconId, inProgress: !earned && Boolean(next && next.current > 0) };
  }).sort((left, right) => Number(right.earned) - Number(left.earned) || Number(right.inProgress) - Number(left.inProgress));
  const rewardsPending = Boolean(props.rewardsLoading || (props.compact && !props.rewards && !props.rewardsError));
  ...
  const previewExperience = (
    <section className="points-preview" aria-label="Signal Points experience">
      ...
      <nav className="points-tabs" role="tablist" aria-label="Signal Points sections">
        {previewTabs.map((item, index) => (
          <button
            key={item}
            ref={(node) => { tabRefs.current[item] = node || undefined; }}
            id={`signal-points-tab-${item}`}
            type="button"
            role="tab"
            aria-selected={tab === item}
            aria-controls={`signal-points-panel-${item}`}
      ...
      <div id="signal-points-panel-overview" className="points-tab-panel" role="tabpanel" aria-labelledby="signal-points-tab-overview" tabIndex={0} hidden={tab !== "overview"}>
        <section className="points-focus-grid">
          ...
          <button className="points-feature-card badge-focus" type="button" onClick={() => setTab("badges")}>
            <span className="points-kicker">{props.rewardsError ? "Badge progress" : "Next achievement"}</span>
            {!rewardsPending && !props.rewardsError && nextBadge && badgeIcon(nextBadge.id) ? <img src={badgeIcon(nextBadge.id) || undefined} alt="" /> : <div className="reward-mark">★</div>}
            <div>
              <strong>{rewardsPending ? "Loading badges" : props.rewardsError ? "Badge progress unavailable" : nextBadge ? `${badgeLabel(nextBadge.id, nextBadge.label)}${nextBadge.tier ? ` · ${nextBadge.tier}` : ""}` : "Badge collection"}</strong>
              <small>{rewardsPending ? "Checking your latest achievement progress." : props.rewardsError ? "Open Badges to try loading progress again." : nextBadge ? `${nextBadge.current} / ${nextBadge.target}` : "All visible achievements earned"}</small>
              {!rewardsPending && !props.rewardsError && nextBadge ? <div className="points-progress" aria-label={`${achievementProgress}% progress`}><span style={{ width: `${achievementProgress}%` }} /></div> : null}
            </div>
            <b>View badges →</b>
          </button>
        </section>
      ...
      <div id="signal-points-panel-rewards" className="points-tab-panel" role="tabpanel" aria-labelledby="signal-points-tab-rewards" tabIndex={0} hidden={tab !== "rewards"}><div className="points-section-heading"><div><span className="points-kicker">Rewards</span><h3>Choose what’s ready</h3></div><small>Physical rewards include U.S. shipping.</small></div><div className="preview-catalog">{visibleCatalog.map((item, index) => {
      ...
      <div id="signal-points-panel-badges" className="points-tab-panel" role="tabpanel" aria-labelledby="signal-points-tab-badges" tabIndex={0} hidden={tab !== "badges"}>
        {rewardsPending ? (
          <div className="points-rewards-state" aria-live="polite"><strong>Loading badge progress…</strong><small>Checking your latest sightings, streaks, and earned tiers.</small></div>
        ) : props.rewardsError ? (
          <><div className="points-rewards-state" role="alert"><strong>Badge progress is unavailable</strong><small>{props.rewardsError}</small><button type="button" onClick={props.onRetryRewards}>Try again</button></div>{badgeCollection}</>
        ) : props.rewards ? <>
          <div className="points-section-heading"><div><span className="points-kicker">Progress</span><h3>Achievements</h3></div><small>{countLabel(props.rewards.eligibleSightings, "sighting")} · {countLabel(props.rewards.currentWeeklyStreak, "week")} streak · {countLabel(props.rewards.badges.length, "badge")} earned</small></div>
      ...
      <div id="signal-points-panel-history" className="points-tab-panel" role="tabpanel" aria-labelledby="signal-points-tab-history" tabIndex={0} hidden={tab !== "history"}><div className="points-section-heading"><div><span className="points-kicker">Redemptions</span><h3>Reward history</h3></div></div>{data.redemptions.length ? <div className="preview-history">{data.redemptions.map((item) => <article key={item.id}><div><strong>{String(item.itemSnapshot.name || item.itemKey)}</strong><small>{new Date(item.createdAt).toLocaleDateString()} · {item.pointsSpent} points</small></div><span>{item.status.replaceAll("_", " ")}</span>{["reserved", "details_required", "submitted", "approved"].includes(item.status) ? <button disabled={saving} type="button" onClick={() => void cancel(item.id)}>Cancel</button> : null}</article>)}</div> : <div className="points-empty"><span>◇</span><strong>No redemptions yet</strong><small>Your history will appear here after your first reward.</small><button type="button" onClick={() => setTab("rewards")}>Explore rewards</button></div>}</div>
  ...
  if (!props.compact) return previewExperience;
  return (
    <section id="signal-points" className="signal-points-accordion" aria-label="Signal Points">
      {compactControl}
      <div id="signal-points-accordion-panel" className="signal-points-accordion-panel" role="region" aria-labelledby="dashboard-section-memberPoints" hidden={!props.expanded}>
...
function AccordionStyles() {
  return <style jsx global>{`
    ...
    .signal-points-accordion-panel{border-radius:0 0 var(--radius-feature) var(--radius-feature);border-bottom:1px solid var(--boundary-accent);background:linear-gradient(145deg,rgba(25,18,11,.9),rgba(12,9,7,.96));padding:18px;overflow:visible}
    .points-rewards-state{display:grid;gap:8px;justify-items:start;border:1px solid rgba(232,201,122,.12);border-radius:18px;background:rgba(5,4,3,.22);padding:24px}
    .points-tab-panel[hidden]{display:none}
    .points-rewards-state strong{font:800 15px/1.3 var(--font-dm-sans);color:var(--color-cream)}
    .points-rewards-state small{font:12px/1.5 var(--font-dm-sans);color:var(--color-text-secondary)}
    .points-rewards-state button{min-height:38px;border:1px solid rgba(232,201,122,.34);border-radius:999px;background:rgba(196,148,58,.12);color:var(--color-accent-amber);padding:9px 14px;font:850 12px/1 var(--font-dm-sans);cursor:pointer}
    .signal-points-accordion-retry{display:grid;gap:10px;justify-items:start}
    .signal-points-accordion-retry p{margin:0;font:12px/1.5 var(--font-dm-sans);color:var(--color-text-secondary)}
    .signal-points-accordion-retry button{min-height:38px;border:1px solid rgba(232,201,122,.34);border-radius:999px;background:rgba(196,148,58,.12);color:var(--color-accent-amber);padding:9px 14px;font:850 12px/1 var(--font-dm-sans);cursor:pointer}
    .signal-points-accordion-retry button:focus-visible{outline:2px solid var(--color-accent-amber);outline-offset:3px}
    @media(max-width:700px){.signal-points-accordion-panel{padding:12px}}
  `}</style>;
}
