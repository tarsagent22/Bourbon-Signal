...
export function encryptOhlqWorkerBlob(value: unknown, secret?: string) {
  ...
  const plaintext = gzipSync(JSON.stringify(value), { level: 9 });
...
export function decryptOhlqWorkerBlob(raw: string, secret?: string) {
  ...
  return JSON.parse(gunzipSync(compressed, { maxOutputLength: 20 * 1024 * 1024 }).toString("utf8")) as Record<string, unknown>;
}
