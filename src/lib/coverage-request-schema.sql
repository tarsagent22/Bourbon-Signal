CREATE TABLE IF NOT EXISTS coverage_requests (
  ...
  target_type TEXT NOT NULL CHECK (target_type IN ('state', 'city', 'store')),
  ...
  CHECK (
    ...
    OR (target_type = 'store' AND store_name IS NOT NULL AND (store_id IS NOT NULL OR area_key IS NOT NULL))
  )
);
