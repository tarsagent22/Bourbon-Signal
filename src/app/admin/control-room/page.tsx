...
async function closeCoverageRequest(formData: FormData) {
  "use server";
  ...
  if (!confirmed) redirect("/admin/control-room?coverageError=Confirm%20the%20request%20closure%20first.#coverage-demand");
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(requestId)) {
    redirect("/admin/control-room?coverageError=That%20coverage%20request%20ID%20is%20invalid.#coverage-demand");
  }

  const result = await getCoverageRequestRepository().closeForOwner(requestId, ownerEmail);
  if (!result) redirect("/admin/control-room?coverageError=That%20request%20is%20already%20closed%20or%20no%20longer%20available.#coverage-demand");
  invalidateCompanyControlRoomSnapshot();
  revalidatePath("/admin/control-room");
  redirect(`/admin/control-room?coverageClosed=${encodeURIComponent(result.requestId)}#coverage-demand`);
}
...
export default async function CompanyControlRoomPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const coverageClosedId = typeof query?.coverageClosed === "string" && /^[0-9a-f-]{36}$/i.test(query.coverageClosed)
    ? query.coverageClosed
    : "";
  ...
  const coverageClosedConfirmed = Boolean(coverageClosedId && coverageDemand.recentRequests.some(
    (request) => request.id === coverageClosedId && request.status === "closed",
  ));
    ...
    <main className="cr-shell">
      ...
      <div className="cr-frame">
        ...
        <section id="coverage-demand" className="cr-section">
          ...
          {coverageClosedConfirmed ? <p className="cr-action-message good" role="status">Coverage request closed.</p> : null}
                    ...
                    {coverageDemand.recentRequests.map((request) => (
                      <article key={request.id}>
                        ...
                        <div className="cr-request-actions">
                          ...
                          {request.status === "requested" || request.status === "on_radar" ? (
                            <details className="cr-close-request">
                              <summary>Manage</summary>
                              <form action={closeCoverageRequest}>
                                <input type="hidden" name="requestId" value={request.id} />
                                <p>Closing moves this request to history and cancels work that has not entered delivery. An Engine Ops update already in flight may still complete. It does not email the requester.</p>
                                <label><input type="checkbox" name="confirmed" value="yes" required /> Confirm closure</label>
